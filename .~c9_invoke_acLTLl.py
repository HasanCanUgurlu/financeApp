...
def index():
    rows = db.execute("SELECT symbol, SUM(shares) as totalShares FROM transactions WHERE user_id=:user_id GROUP BY symbol HAVING Shares > 0;",
...
def buy():
    if request.method == "POST":
        if is_provided("symbol") or is_provided("shares"):
            return
        elif not request.form.get("shares").isdigit() or not request.form.get("shares"):
            return apology("invalid number", 400)
        symbol = request.form.get("symbol").upper()
        shares = int(request.form.get("shares"))
        stock = lookup(symbol)
        if stock is None:
            return apology("Couldn't find the symbol", 400)
...
def quote():
    if request.method == "POST":
        ...
        stock = lookup(symbol)
        if stock is None:
            return apology("Couldn't find stock symbol", 400)
...
def register():
    """Register user"""
    if request.method == "POST":
        check = is_provided("username") or is_provided("password") or is_provided("confirmation")
        if check:
            return check
